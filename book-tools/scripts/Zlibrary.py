...
class ZlibraryAPIError(RuntimeError):
    """Raised when an EAPI endpoint is unreachable or returns an invalid response."""


DEFAULT_EAPI_DOMAINS = ("z-library.ec", "z-lib.sk", "z-library.sk", "1lib.sk")
...
class Zlibrary:
    def __init__(
        ...
        self.__domain = (
            self.__normalizeDomain(domain) if domain else self.__selectDomain()
        )
    ...
    def __setValues(self, response) -> dict[str, str]:
        ...
        if not isinstance(user, dict) or "id" not in user or "remix_userkey" not in user:
            raise ZlibraryAPIError("Z-Library returned an incomplete user profile")
        ...
        self.__remix_userkey = user["remix_userkey"]
    ...
    @staticmethod
    def __normalizeDomain(domain: str) -> str:
        value = domain.strip()
        if not value:
            raise ZlibraryAPIError("Z-Library EAPI domain cannot be empty")
        if "://" not in value:
            value = "https://" + value
        parsed = urlparse(value)
        if parsed.scheme != "https" or not parsed.netloc:
            raise ZlibraryAPIError(
                "Z-Library EAPI domain must be an HTTPS hostname, for example https://z-library.ec"
            )
        return parsed.netloc

    def __selectDomain(self) -> str:
        errors = []
        for domain in DEFAULT_EAPI_DOMAINS:
            try:
                ...
                payload = self.__decodeJSON(response, domain, "/eapi/info/domains")
                if payload.get("success"):
                    return domain
                errors.append(f"{domain}: API reported failure")
            except ZlibraryAPIError as exc:
                errors.append(f"{domain}: {exc}")
            except requests.RequestException as exc:
                errors.append(f"{domain}: {exc.__class__.__name__}")
        raise ZlibraryAPIError(
            "No working Z-Library EAPI domain found (" + "; ".join(errors) + ")"
        )

    @staticmethod
    def __decodeJSON(response, domain: str, endpoint: str) -> dict:
        content_type = response.headers.get("content-type", "").lower()
        if not 200 <= response.status_code < 300:
            raise ZlibraryAPIError(
                f"Z-Library EAPI returned HTTP {response.status_code} from {domain}{endpoint}"
            )
        if "json" not in content_type:
            raise ZlibraryAPIError(
                f"Z-Library EAPI returned non-JSON content ({content_type or 'unknown type'}) "
                f"from {domain}{endpoint}"
            )
        try:
            payload = response.json()
        except ValueError as exc:
            raise ZlibraryAPIError(
                f"Z-Library EAPI returned malformed JSON from {domain}{endpoint}"
            ) from exc
        if not isinstance(payload, dict):
            raise ZlibraryAPIError(
                f"Z-Library EAPI returned an unexpected JSON value from {domain}{endpoint}"
            )
        return payload

    def __requestJSON(self, method: str, url: str, **kwargs) -> dict:
        try:
            ...
            raise ZlibraryAPIError(
                f"Could not reach Z-Library EAPI at {self.__domain}{url}: {exc.__class__.__name__}"
            ) from exc
        return self.__decodeJSON(response, self.__domain, url)

    def __makePostRequest(
        self, url: str, data: dict = None, override=False
    ) -> dict[str, str]:
    ...
    def __makeGetRequest(
        self, url: str, params: dict = None, cookies=None
    ) -> dict[str, str]:
