...
The vendored `Zlibrary.py` probes `z-library.ec`, `z-lib.sk`, `z-library.sk`, and `1lib.sk`, selecting the first endpoint that returns valid EAPI JSON. Set `ZLIBRARY_EAPI_DOMAIN` in the canonical `.env` or use `config set --zlib-domain` to pin an HTTPS domain.
